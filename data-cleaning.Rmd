...
attack_tweets_west_final <- readRDS("attack_tweets_west_final.RDS")
attack_tweets_manc_final <- readRDS("attack_tweets_manc_final.RDS")
attack_tweets_lond_final <- readRDS("attack_tweets_lond_final.RDS")
attack_tweets_fins_final <- readRDS("attack_tweets_fins_final.RDS")
attack_tweets_pars_final <- readRDS("attack_tweets_pars_final.RDS")
```

Cut off the last three hours so to have even number of hours for analysis (48)

```{r}
attack_tweets_west_final2 <- attack_tweets_west_final %>% 
  filter(created_ymdhourly <= ymd_hm("2017-03-24 11:40"))

attack_tweets_manc_final2 <- attack_tweets_manc_final %>% 
  filter(created_ymdhourly <= ymd_hm("2017-05-24 18:30"))

attack_tweets_lond_final2 <- attack_tweets_lond_final %>% 
  filter(created_ymdhourly <= ymd_hm("2017-06-05 18:06"))

attack_tweets_fins_final2 <- attack_tweets_fins_final %>% 
  filter(created_ymdhourly <= ymd_hm("2017-06-20 21:05"))  

attack_tweets_pars_final2 <- attack_tweets_pars_final %>% 
  filter(created_ymdhourly <= ymd_hm("2017-09-17 04:20"))  
```

```{r}
# saveRDS(attack_tweets_west_final2, "attack_tweets_west_final2.RDS")
# saveRDS(attack_tweets_manc_final2, "attack_tweets_manc_final2.RDS")
# saveRDS(attack_tweets_lond_final2, "attack_tweets_lond_final2.RDS")
# saveRDS(attack_tweets_fins_final2, "attack_tweets_fins_final2.RDS")
# saveRDS(attack_tweets_pars_final2, "attack_tweets_pars_final2.RDS")
```

```{r}
length(unique(attack_tweets_west_final2$author_id))
length(unique(attack_tweets_west_final2$id))

length(unique(attack_tweets_manc_final2$author_id))
length(unique(attack_tweets_manc_final2$id))

length(unique(attack_tweets_lond_final2$author_id))
length(unique(attack_tweets_lond_final2$id))

length(unique(attack_tweets_fins_final2$author_id))
length(unique(attack_tweets_fins_final2$id))

length(unique(attack_tweets_pars_final2$author_id))
length(unique(attack_tweets_pars_final2$id))
```
