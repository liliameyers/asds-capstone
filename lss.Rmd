...
```{r cache=TRUE}
attack_tweets_west_final <- readRDS("attack_tweets_west_final2.RDS")
attack_tweets_manc_final <- readRDS("attack_tweets_manc_final2.RDS")
attack_tweets_lond_final <- readRDS("attack_tweets_lond_final2.RDS")
attack_tweets_fins_final <- readRDS("attack_tweets_fins_final2.RDS")
attack_tweets_pars_final <- readRDS("attack_tweets_pars_final2.RDS")
```

Combine all attacks for analysis 

```{r cache=TRUE, eval=FALSE}
attack_tweets_west_final$attack <- "Westminster"
attack_tweets_manc_final$attack <- "Manchester Arena"
attack_tweets_lond_final$attack <- "London Bridge"
attack_tweets_fins_final$attack <- "Finsbury Park"
attack_tweets_pars_final$attack <- "Parsons Green"

attack_tweets_alll_final <- rbind(attack_tweets_west_final,
                                  attack_tweets_manc_final,
                                  attack_tweets_lond_final,
                                  attack_tweets_fins_final,
                                  attack_tweets_pars_final)

# saveRDS(attack_tweets_alll_final, "attack_tweets_alll_final.RDS")
```
...
London - multiculturalism (but used by ann coulder), dumped, borders, control, #travelban
Parsons - deport
...
# 1a: all words from  qulitative undestanding 
seedword_dict_1a <- dictionary(list(
  positive = c("islamaphobia", "love", "together", "racist", "#refugeeswelcome", "tories", "welcome", "racists",
               "empathy", "positive", "benefit", "amazing", "rise", "vulnerable", "wages"),
  negative = c("disgraceful", "murders", "ban", "#buildthewall","deport", "control", "#travelban", "blood", "thug", 
               "bad", "nasty", "wrong", "illegals", "illegal", "horrible", "evil", "problem", "mass", "dumped", "destroyed",
               "close", "extremist", "brexit", "control", "coming", "vet", "#closetheborders", "stop")
  ))

seedword_dict_1b <- dictionary(list(
  positive = c("islamaphobia", "love", "together", "racist", "#refugeeswelcome", "tories", "welcome",
               "empathy", "positive", "benefit", "amazing", "vulnerable"),
  negative = c("disgraceful", "murderers", "ban", "#buildthewall","deport", "control", "#travelban", "blood", "thug", 
               "bad", "nasty", "wrong", "illegals", "illegal", "horrible", "evil", "problem", "mass", "dumped", "destroyed",
               "extremist", "control", "vet", "#closetheborders", "killers")
  ))

seedword_dict_1c <- dictionary(list(
  positive = c("islamaphobia", "love", "together", "racist", "#refugeeswelcome", "tories", "welcome",
               "empathy", "positive", "benefit", "amazing", "vulnerable"),
  negative = c("disgraceful", "#buildthewall","deport", "control", "#travelban", "blood", "thug", "disgusting",
               "illegals", "evil", "problem", "mass", "dumped", "destroyed", "vet", "#closetheborders")
  ))
```
...
```{r cache=TRUE}
lss_alll_output <- lss_function(corpus_alll, seedword_dict_1c, k_val = 300)
...
print(textplot_terms(lss_mod_alll, seedword_dict_1c))
...
ggsave(lss_polarity_neg, file = "lss_polarity_neg.png", height = 8, width = 10)
ggsave(lss_polarity_pos, file = "lss_polarity_pos.png", height = 8, width = 10)
```
...
coefs_alll %>% 
  filter(word %in% seedword_dict_1c[["positive"]] 
         | word %in% seedword_dict_1c[["negative"]])
...
```{r}
ggsave(lss_plot_all, file = "lss_plots_all_1c.png", height = 7, width = 10)
```
...
factor_plot_leg_adj <- shift_legend2(factor_plot)
...
ggsave(file = "file1.png", shift_legend3(factor_plot), height = 7, width = 10)
```
...
```{r}
ggsave(file = "file2.png", shift_legend3(factor_plot), height = 7, width = 10)
```
```{r}
...
```{r}
ggsave(file = "file2.png", shift_legend3(dist_plot), height = 7, width = 10)
```

***********************************************************************************************************************

Get table for initial and final distributions
...
# saveRDS(plot_df_dens_initial, "initial_opinions.RDS")
```
